...
<link rel="stylesheet" href="bmi-edit.css">
<title>BMI Entry Edit Output</title>
</head>
<body>
	<%
		String date = request.getParameter("bmi-edit-date");
		System.out.println("BMI_Edit_Output.jsp: date = " + date);
		
		String bmiLogID = request.getParameter("bmi-edit-bmiLogID");
		System.out.println("BMI_Edit_Output.jsp: bmiLogID = " + bmiLogID);

		String num = request.getParameter("bmi-edit-num");
		System.out.println("BMI_Edit_Output: num = " + num);
		
		String bmi = request.getParameter("bmi-edit-bmi");
		System.out.println("BMI_Edit_Output: old bmi = " + bmi);
		
		String sHeight = request.getParameter("height-input");
		String sWeight = request.getParameter("weight-input");
		...
		if (height < 0 || weight < 0) {
			...
			//output confirmation
			out.print("<script> alert('Successfully Saved!'); window.location='BMIEntry_Search.jsp';</script>");
			
			BMIEntry b = new BMIEntry(height, weight, date);
			...
			BMIEntryDBHelper bdbhelp = new BMIEntryDBHelper();
			boolean delete = bdbhelp.deleteEntry(idC.getValue(), bmiLogID);			//deletes old entry
			//only insert edited entry if delete is successful
			if(delete){
				System.out.println("BMI_Edit_Output.jsp: delete successful");
				bdbhelp.addEntry(idC.getValue(), b);								//replaces w/ new entry (same date)
			}
			
			bmi = b.getBMI() + "";
			System.out.println("BMI_Edit_Output: new bmi = " + bmi);
		}
	%>
	...
	<div class="main">
		...
		<div class="main-body">
			<form action="/" class="body-text">
				<div class="bmi-body">
					BMI LOG #<%out.print(num);%> <br>
					<div id="date">
						 <%out.print(date);%>
					</div>
					<div id="old-bmi-header">
						New BMI: <%out.print(bmi);%>
					</div>
				</div>
				<table id="table1">
					<tr>
						<th class="th1">New Height
							<div id="unit">(in inches)</div>
						</th>
						<th><input type="text" name="height-input"> <br></th>
					</tr>
					<tr>
						<th class="th1">New Weight
							<div id="unit">(in pounds)</div>
						</th>
						<th><input type="text" name="weight-input">
							<br></th>
					</tr>
				</table>
