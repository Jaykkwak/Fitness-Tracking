...
<body>
	...
	<%
		String sheight = request.getParameter("settings-height");
		String sweight = request.getParameter("settings-weight");
		String stweight = request.getParameter("settings-target-weight");
		String stframe = request.getParameter("settings-tframe");
		
		int height = Integer.valueOf(sheight);
		int weight = Integer.valueOf(sweight);
		int tweight = Integer.valueOf(stweight);
		int tframe = Integer.valueOf(stframe);
		
		if(height > 0 && weight > 0 && tweight > 0 && tframe > 0){
			Cookie idC = null;
			...
			FitnessGoal fg = new FitnessGoal(height, weight, tweight, tframe); 
			FitnessGoalDBHelper fgdbhelp = new FitnessGoalDBHelper();
			System.out.println("Settings_FGOut.jsp: userID = " + idC.getValue());
			
			boolean delete = fgdbhelp.deleteEntry(idC.getValue());
			
			System.out.println(idC.getValue());
			
			boolean newFG = false;
			if(delete){
				newFG = fgdbhelp.addEntry(idC.getValue(), fg);
			}
			
			System.out.println(delete + ", " + newFG);
			
			if(delete && newFG){
				System.out.println("Settings_FGOut.jsp: FG changed successfully");
				out.print("<script> alert('Saved Successfully!');  window.location = 'Settings.jsp';</script>");
			}
			else{
				System.out.println("Settings_FGOut.jsp: something went wrong");
				out.print("<script> alert('Invalid Input\\nPlease Try Again'); window.location = document.referrer;</script>");
			}
		}
		else{
			System.out.println("Settings_FGOut.jsp: invalid input");
			out.print("<script> alert('Invalid Input\\nPlease Try Again'); window.location = document.referrer;</script>");
		}
	%>
