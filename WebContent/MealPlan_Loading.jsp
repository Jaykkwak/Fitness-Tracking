...
<body>
    ...
    <%
		...
		for (Cookie c : cookies) {
			...
			if(exists){
				System.out.println("MP_Load.jsp: oldmp exists");
				int intake = oldmp.getIntake();
				int targetBmi = oldmp.getTargetBMI();
				int protein = oldmp.getProtein();
				int carbs = oldmp.getCarbs();
				int fat = oldmp.getFat();
				
				String values = "intake="+intake+"&bmi="+targetBmi+"&protein="+protein+"&carbs="+carbs+"&fat="+fat;
				out.print("<script> window.location = 'MealPlan_Edit_input.jsp?"+values+"';</script>");
			}
			else{
				System.out.println("MP_Load.jsp: no mp yet");
				out.print("<script> window.location = 'MealPlan_Input.jsp';</script>");
			}
    %>
