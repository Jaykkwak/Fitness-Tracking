...
<body>
	<%
		...
		if (reps1 < 0 || sets1 < 0 || weight1 < 0 || reps2 < 0 || sets2 < 0 || weight2 < 0 ||
			...
			WorkoutEntryDBHelper wdbhelp = new WorkoutEntryDBHelper();
			
			boolean delete = wdbhelp.deleteEntry(idC.getValue(), oldWorkoutLogID);
			if(delete){
				wdbhelp.addEntry(idC.getValue(), w);
				//output confirmation
				out.print("<script> alert('Successfully Saved!'); window.location='WorkoutEntry_Search.jsp'</script>");
			}
			else{
				//output confirmation
				out.print("<script> alert('Something Went Wrong\\nPlease Try Again'); window.location=document.referrer; </script>");
			}
		}
	%>
